...
def generate_random_edges(
    verts: int, num_rand_edges: int
) -> tuple[set[Edge], set[FrozenSet[int]], int]:
    """
    ...
    random_edge_indices = np.random.choice(
        len(list(combinations(path_seq, 2))), size=num_rand_edges
    )
...
def generate_hampath_graph(verts: int, num_rand_edges) -> tuple[Data, int]:
    ...
    edges_transposed = torch.tensor(
        np.array(list(map(list, generate_undirected_graph(all_edges))))
    )
    edge_index = edges_transposed.t().contiguous()

    data = Data(edge_index=edge_index, num_nodes=verts)
    pos_encoder = AddLaplacianEigenvectorPE(
        POS_ENC_DIM, is_undirected=True, attr_name="x"
    )
    pos_encoder(data)
...
def generate_semirandom_hampath_graph(
    verts: int, delta_v: int, num_rand_edges: int, delta_e: int
) -> tuple[Data, int]:
...
def generate_and_save_corpus(
    ...
    for i in tqdm(range(num_graphs)):
        data = generate_semirandom_hampath_graph(
            verts, delta_v, num_rand_edges, delta_e
        )
